...
/* cardano-c: portable no-assembly build. Use 32-bit limbs with a 64-bit
 * double-limb (the reference C path in no_asm.h), which compiles on every
 * target including WASM and the armv6/armv7 cross builds. This branch is
 * tested first so that forcing __BLST_NO_ASM__ on x86_64/aarch64 selects the
 * portable C path instead of the (intentionally absent) assembly modules. */
