...
TEST(cardano_input_to_redeemer_map_insert, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_input_to_redeemer_map_t* map      = nullptr;
  cardano_redeemer_t*              redeemer = new_default_redeemer();

  EXPECT_EQ(cardano_input_to_redeemer_map_new(&map), CARDANO_SUCCESS);
  ...
  for (size_t i = 0U; i < 31U; ++i)
  {
    ...
    EXPECT_EQ(cardano_transaction_input_new(hash, 0U, &input), CARDANO_SUCCESS);

    EXPECT_EQ(cardano_input_to_redeemer_map_insert(map, input, redeemer), CARDANO_SUCCESS);
  ...
  const size_t input_ref_count    = cardano_transaction_input_refcount(input);
  ...
  // Act
  cardano_error_t result = cardano_input_to_redeemer_map_insert(map, input, redeemer);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_input_to_redeemer_map_get_length(map), 31U);
  EXPECT_EQ(cardano_transaction_input_refcount(input), input_ref_count);
  EXPECT_EQ(cardano_redeemer_refcount(redeemer), redeemer_ref_count);

  // Cleanup
  cardano_set_allocators(malloc, realloc, free);
  reset_limited_realloc();
  cardano_input_to_redeemer_map_unref(&map);
  cardano_blake2b_hash_unref(&hash);
  cardano_transaction_input_unref(&input);
  cardano_redeemer_unref(&redeemer);
}
