...
TEST(cardano_voting_procedure_list_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_voting_procedure_list_t* list    = nullptr;
  cardano_voting_procedure_t*      element = new_default_voting_procedure(VOTING_PROCEDURE_CBOR_1);

  EXPECT_EQ(cardano_voting_procedure_list_new(&list), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_voting_procedure_list_add(list, element), CARDANO_SUCCESS);
  }

  const size_t ref_count = cardano_voting_procedure_refcount(element);
  ...
  // Act
  cardano_error_t result = cardano_voting_procedure_list_add(list, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_voting_procedure_list_get_length(list), 127U);
  EXPECT_EQ(cardano_voting_procedure_refcount(element), ref_count);
