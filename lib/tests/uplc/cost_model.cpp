...
// Protocol version 11 (van Rossem) unified the builtin set across the three
// languages, so the V1 defaults now price serialise_data like V2 instead of
// carrying the unavailable sentinel.
TEST(cardano_uplc_builtin_cost, v1_serialise_data_matches_v2_after_unification)
{
  cardano_uplc_arena_t* arena = nullptr;
  ASSERT_EQ(cardano_uplc_arena_new(4096U, &arena), CARDANO_SUCCESS);

  cardano_uplc_builtin_costs_t costs_v1 = cardano_uplc_builtin_costs_v1();
  cardano_uplc_builtin_costs_t costs_v2 = cardano_uplc_builtin_costs_v2();

  const cardano_uplc_value_t* args[1];
  args[0] = make_bytes_value(arena, 8U);

  cardano_uplc_budget_t budget_v1 = cardano_uplc_builtin_cost(&costs_v1, CARDANO_UPLC_BUILTIN_SERIALISE_DATA, args, 1U, false);
  cardano_uplc_budget_t budget_v2 = cardano_uplc_builtin_cost(&costs_v2, CARDANO_UPLC_BUILTIN_SERIALISE_DATA, args, 1U, false);

  EXPECT_LT(budget_v1.cpu, 30000000000);
  EXPECT_EQ(budget_v1.cpu, budget_v2.cpu);
  EXPECT_EQ(budget_v1.mem, budget_v2.mem);
