...
TEST(cardano_required_guards_map_get_keys, returnsErrorIfRequiredGuardsMapIsNull)
{
  // Arrange
  cardano_credential_set_t* keys = nullptr;

  // Act
  cardano_error_t error = cardano_required_guards_map_get_keys(nullptr, &keys);

  // Assert
  EXPECT_EQ(error, CARDANO_ERROR_POINTER_IS_NULL);
}

TEST(cardano_required_guards_map_get_keys, returnsErrorIfKeysIsNull)
{
  // Arrange
  cardano_required_guards_map_t* required_guards_map = nullptr;
  cardano_error_t                error               = cardano_required_guards_map_new(&required_guards_map);
  ...
  // Act
  error = cardano_required_guards_map_get_keys(required_guards_map, nullptr);
...
TEST(cardano_required_guards_map_get_keys, returnsEmptySetIfNoElements)
{
  // Arrange
  cardano_required_guards_map_t* required_guards_map = nullptr;
  cardano_error_t                error               = cardano_required_guards_map_new(&required_guards_map);
  ...
  // Act
  error = cardano_required_guards_map_get_keys(required_guards_map, &keys);
...
TEST(cardano_required_guards_map_get_keys, returnsTheKeys)
{
  // Arrange
  cardano_required_guards_map_t* required_guards_map = new_default_required_guards_map(CBOR);
  ASSERT_THAT(required_guards_map, testing::Not((cardano_required_guards_map_t*)nullptr));
  ...
  // Act
  cardano_error_t error = cardano_required_guards_map_get_keys(required_guards_map, &keys);

  // Assert
  EXPECT_EQ(error, CARDANO_SUCCESS);
  EXPECT_EQ(cardano_credential_set_get_length(keys), 2);

  const char* expected_credentials[] = { KEY_HASH_CREDENTIAL_CBOR, SCRIPT_HASH_CREDENTIAL_CBOR };
