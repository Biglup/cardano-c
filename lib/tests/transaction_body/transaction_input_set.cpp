...
TEST(cardano_transaction_input_set_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_transaction_input_set_t* set     = nullptr;
  cardano_transaction_input_t*     element = new_default_transaction_input(TRANSACTION_INPUT1_CBOR);

  EXPECT_EQ(cardano_transaction_input_set_new(&set), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_transaction_input_set_add(set, element), CARDANO_SUCCESS);
  }

  const size_t ref_count = cardano_transaction_input_refcount(element);
  ...
  // Act
  cardano_error_t result = cardano_transaction_input_set_add(set, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_transaction_input_set_get_length(set), 127U);
  EXPECT_EQ(cardano_transaction_input_refcount(element), ref_count);
...
TEST(cardano_transaction_input_set_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_transaction_input_set_t* set     = nullptr;
  cardano_transaction_input_t*     element = new_default_transaction_input(TRANSACTION_INPUT1_CBOR);
  ...
  EXPECT_EQ(cardano_transaction_input_set_to_cbor(set, writer), CARDANO_SUCCESS);
  ...
  cardano_transaction_input_set_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_transaction_input_set_from_cbor(reader, &decoded);
