...
TEST(cardano_stake_vote_delegation_cert_to_cip116_json, canConvertToCip116Json)
{
  // Arrange
  cardano_stake_vote_delegation_cert_t* cert = new_default_cert();
  ...
  // Act
  cardano_error_t error    = cardano_stake_vote_delegation_cert_to_cip116_json(cert, json);
  char*           json_str = encode_json(json);

  // Assert
  EXPECT_EQ(error, CARDANO_SUCCESS);

  const char* expected = R"({"tag":"stake_vote_delegation","credential":{"tag":"pubkey_hash","value":"00000000000000000000000000000000000000000000000000000000"},"pool_keyhash":"00000000000000000000000000000000000000000000000000000000","drep":{"tag":"pubkey_hash","value":"00000000000000000000000000000000000000000000000000000000"}})";
...
TEST(cardano_stake_vote_delegation_cert_to_cip116_json, returnsErrorIfCertIsNull)
{
  cardano_json_writer_t* json  = cardano_json_writer_new(CARDANO_JSON_FORMAT_COMPACT);
  cardano_error_t        error = cardano_stake_vote_delegation_cert_to_cip116_json(nullptr, json);
  EXPECT_EQ(error, CARDANO_ERROR_POINTER_IS_NULL);
  cardano_json_writer_unref(&json);
}

TEST(cardano_stake_vote_delegation_cert_to_cip116_json, returnsErrorIfWriterIsNull)
{
  // Arrange
  cardano_stake_vote_delegation_cert_t* cert = new_default_cert();

  // Act
  cardano_error_t error = cardano_stake_vote_delegation_cert_to_cip116_json(cert, nullptr);
