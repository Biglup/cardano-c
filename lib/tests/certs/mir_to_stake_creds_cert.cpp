...
TEST(cardano_mir_to_stake_creds_cert_insert, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_mir_to_stake_creds_cert_t* cert = nullptr;

  EXPECT_EQ(cardano_mir_to_stake_creds_cert_new(CARDANO_MIR_CERT_POT_TYPE_RESERVE, &cert), CARDANO_SUCCESS);
  ...
  for (size_t i = 0U; i < 127U; ++i)
  {
    ...
    EXPECT_EQ(cardano_mir_to_stake_creds_cert_insert(cert, credential, 1), CARDANO_SUCCESS);
  ...
  // Act
  cardano_error_t result = cardano_mir_to_stake_creds_cert_insert(cert, credential, 1);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_mir_to_stake_creds_cert_get_size(cert), 127U);
...
TEST(cardano_mir_to_stake_creds_cert_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_mir_to_stake_creds_cert_t* cert = nullptr;
  ...
  EXPECT_EQ(cardano_mir_to_stake_creds_cert_to_cbor(cert, writer), CARDANO_SUCCESS);
  ...
  cardano_mir_to_stake_creds_cert_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_mir_to_stake_creds_cert_from_cbor(reader, &decoded);
