...
{
  EXPECT_EQ(cardano_asset_id_list_new(nullptr), CARDANO_ERROR_POINTER_IS_NULL);
}
...
TEST(cardano_asset_id_list_new, returnErrorIfMemoryAllocationFails)
{
  ...
  EXPECT_EQ(cardano_asset_id_list_new(&list), CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
...
{
  // Act
  EXPECT_EQ(cardano_asset_id_list_get(nullptr, 0, nullptr), CARDANO_ERROR_POINTER_IS_NULL);
}

TEST(cardano_asset_id_list_get, returnsNullIfElementIsNull)
{
  // Act
  EXPECT_EQ(cardano_asset_id_list_get((cardano_asset_id_list_t*)"", 0, nullptr), CARDANO_ERROR_POINTER_IS_NULL);
}
...
TEST(cardano_asset_id_list_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_asset_id_list_t* list    = nullptr;
  cardano_asset_id_t*      element = new_default_asset_id(ASSET_ID_HEX_1);

  EXPECT_EQ(cardano_asset_id_list_new(&list), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_asset_id_list_add(list, element), CARDANO_SUCCESS);
  }

  const size_t ref_count = cardano_asset_id_refcount(element);
  ...
  // Act
  cardano_error_t result = cardano_asset_id_list_add(list, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_asset_id_list_get_length(list), 127U);
  EXPECT_EQ(cardano_asset_id_refcount(element), ref_count);
