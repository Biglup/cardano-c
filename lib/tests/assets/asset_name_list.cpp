...
{
  EXPECT_EQ(cardano_asset_name_list_new(nullptr), CARDANO_POINTER_IS_NULL);
}
...
TEST(cardano_asset_name_list_new, returnErrorIfMemoryAllocationFails)
{
  ...
  EXPECT_EQ(cardano_asset_name_list_new(&list), CARDANO_MEMORY_ALLOCATION_FAILED);
...
{
  // Act
  EXPECT_EQ(cardano_asset_name_list_get(nullptr, 0, nullptr), CARDANO_POINTER_IS_NULL);
}

TEST(cardano_asset_name_list_get, returnsNullIfElementIsNull)
{
  // Act
  EXPECT_EQ(cardano_asset_name_list_get((cardano_asset_name_list_t*)"", 0, nullptr), CARDANO_POINTER_IS_NULL);
}
