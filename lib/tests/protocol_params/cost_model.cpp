...
TEST(cardano_cost_model_new, canCreateCostModelV4)
{
  // Arrange
  cardano_cost_model_t* cost_model = nullptr;
  std::vector<int64_t>  costs      = hex_string_to_costs(COST_MODEL_V4_HEX);

  // Act
  cardano_error_t error = cardano_cost_model_new(CARDANO_PLUTUS_LANGUAGE_VERSION_V4, &costs[0], costs.size(), &cost_model);
...
TEST(cardano_cost_model_from_cbor, canDeserializeCostModelV4)
{
  // Arrange
  cardano_cost_model_t*  cost_model = nullptr;
  ...
  EXPECT_EQ(cardano_cost_model_get_language(cost_model, &language), CARDANO_SUCCESS);
  EXPECT_EQ(language, CARDANO_PLUTUS_LANGUAGE_VERSION_V4);
