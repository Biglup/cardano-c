...
{
  // Act
  EXPECT_EQ(cardano_native_script_set_set_use_tag(nullptr, true), CARDANO_ERROR_POINTER_IS_NULL);
}
...
TEST(cardano_native_script_set_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_native_script_set_t* set     = nullptr;
  cardano_native_script_t*     element = new_default_native_script(NATIVE_SCRIPT1_CBOR);

  EXPECT_EQ(cardano_native_script_set_new(&set), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_native_script_set_add(set, element), CARDANO_SUCCESS);
  }

  const size_t ref_count = cardano_native_script_refcount(element);
  ...
  // Act
  cardano_error_t result = cardano_native_script_set_add(set, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_native_script_set_get_length(set), 127U);
  EXPECT_EQ(cardano_native_script_refcount(element), ref_count);
...
TEST(cardano_native_script_set_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_native_script_set_t* set     = nullptr;
  cardano_native_script_t*     element = new_default_native_script(NATIVE_SCRIPT1_CBOR);
  ...
  EXPECT_EQ(cardano_native_script_set_to_cbor(set, writer), CARDANO_SUCCESS);
  ...
  cardano_native_script_set_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_native_script_set_from_cbor(reader, &decoded);
