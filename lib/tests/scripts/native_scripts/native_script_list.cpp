...
TEST(cardano_native_script_list_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_native_script_list_t* list    = nullptr;
  cardano_native_script_t*      element = nullptr;

  EXPECT_EQ(cardano_native_script_from_json(ALL_SCRIPT, strlen(ALL_SCRIPT), &element), CARDANO_SUCCESS);

  EXPECT_EQ(cardano_native_script_list_new(&list), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_native_script_list_add(list, element), CARDANO_SUCCESS);
  }
  ...
  // Act
  cardano_error_t result = cardano_native_script_list_add(list, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_native_script_list_get_length(list), 127U);
...
TEST(cardano_native_script_list_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_native_script_list_t* list    = nullptr;
  cardano_native_script_t*      element = nullptr;
  ...
  cardano_native_script_list_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_native_script_list_from_cbor(reader, &decoded);
