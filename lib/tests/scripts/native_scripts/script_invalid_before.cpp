...
/**
 * Serializes a script_invalid_before and returns its CBOR as a hex string.
 *
 * @param script The script_invalid_before to serialize.
 *
 * @return The CBOR hex string. The caller must release it with free.
 */
static char*
script_invalid_before_to_cbor_hex(const cardano_script_invalid_before_t* script)
{
  cardano_cbor_writer_t* writer = cardano_cbor_writer_new();

  EXPECT_EQ(cardano_script_invalid_before_to_cbor(script, writer), CARDANO_SUCCESS);
...
/**
 * Reads a script_invalid_before from a CBOR hex string.
 *
 * @param cbor_hex The CBOR hex string.
 *
 * @return The script_invalid_before object.
 */
static cardano_script_invalid_before_t*
script_invalid_before_from_cbor_hex(const char* cbor_hex)
{
  cardano_cbor_reader_t*           reader = cardano_cbor_reader_from_hex(cbor_hex, strlen(cbor_hex));
  cardano_script_invalid_before_t* script = NULL;

  EXPECT_EQ(cardano_script_invalid_before_from_cbor(reader, &script), CARDANO_SUCCESS);
...
/**
 * Releases a script_invalid_before and a hex string returned by script_invalid_before_to_cbor_hex.
 *
 * @param script The script_invalid_before to release.
 * @param hex The hex string to release.
 */
static void
script_invalid_before_unref_and_free(cardano_script_invalid_before_t** script, char* hex)
{
  cardano_script_invalid_before_unref(script);
  free(hex);
}
...
TEST(cardano_script_invalid_before_from_cbor, preservesTheOriginalCborEncoding)
{
  // Arrange
  cardano_script_invalid_before_t* script = script_invalid_before_from_cbor_hex(NON_MINIMAL_INVALID_BEFORE_CBOR);

  // Act
  char* hex = script_invalid_before_to_cbor_hex(script);

  // Assert
  EXPECT_STREQ(hex, NON_MINIMAL_INVALID_BEFORE_CBOR);

  // Cleanup
  script_invalid_before_unref_and_free(&script, hex);
}

TEST(cardano_script_invalid_before_set_slot, discardsTheCachedCbor)
{
  // Arrange
  cardano_script_invalid_before_t* script = script_invalid_before_from_cbor_hex(NON_MINIMAL_INVALID_BEFORE_CBOR);

  // Act
  EXPECT_EQ(cardano_script_invalid_before_set_slot(script, 9U), CARDANO_SUCCESS);

  char* hex = script_invalid_before_to_cbor_hex(script);

  // Assert
  EXPECT_STREQ(hex, "820409");

  // Cleanup
  script_invalid_before_unref_and_free(&script, hex);
}

TEST(cardano_script_invalid_before_clear_cbor_cache, encodesTheScriptFromItsFields)
{
  // Arrange
  cardano_script_invalid_before_t* script = script_invalid_before_from_cbor_hex(NON_MINIMAL_INVALID_BEFORE_CBOR);

  // Act
  cardano_script_invalid_before_clear_cbor_cache(script);
  char* hex = script_invalid_before_to_cbor_hex(script);

  cardano_script_invalid_before_clear_cbor_cache(script);
  char* hex_after_second_clear = script_invalid_before_to_cbor_hex(script);

  // Assert
  EXPECT_STREQ(hex, MINIMAL_INVALID_BEFORE_CBOR);
  EXPECT_STREQ(hex_after_second_clear, MINIMAL_INVALID_BEFORE_CBOR);
...
TEST(cardano_script_invalid_before_clear_cbor_cache, doesntCrashIfGivenANullPtr)
{
  // Act
  cardano_script_invalid_before_clear_cbor_cache(nullptr);
}

TEST(cardano_script_invalid_before_from_cbor, returnsErrorIfMemoryAllocationFailsWhileCachingTheCbor)
{
  // Arrange
  cardano_cbor_reader_t* reader    = cardano_cbor_reader_from_hex(NON_MINIMAL_INVALID_BEFORE_CBOR, strlen(NON_MINIMAL_INVALID_BEFORE_CBOR));
  ...
  for (int i = 0; (i < 500) && !succeeded; ++i)
  {
    ...
    cardano_error_t result = cardano_script_invalid_before_from_cbor(reader_copy, &script);
