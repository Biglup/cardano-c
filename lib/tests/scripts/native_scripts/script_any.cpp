...
static const char* NON_MINIMAL_ANY_CBOR = "82028282041b000000000000000582051b0000000000000007";
static const char* MINIMAL_ANY_CBOR     = "820282820405820507";

/* STATIC FUNCTIONS **********************************************************/

/**
 * Serializes a script_any and returns its CBOR as a hex string.
 *
 * @param script The script_any to serialize.
 *
 * @return The CBOR hex string. The caller must release it with free.
 */
static char*
script_any_to_cbor_hex(const cardano_script_any_t* script)
{
  cardano_cbor_writer_t* writer = cardano_cbor_writer_new();

  EXPECT_EQ(cardano_script_any_to_cbor(script, writer), CARDANO_SUCCESS);
...
/**
 * Reads a script_any from a CBOR hex string.
 *
 * @param cbor_hex The CBOR hex string.
 *
 * @return The script_any object.
 */
static cardano_script_any_t*
script_any_from_cbor_hex(const char* cbor_hex)
{
  cardano_cbor_reader_t* reader = cardano_cbor_reader_from_hex(cbor_hex, strlen(cbor_hex));
  cardano_script_any_t*  script = NULL;

  EXPECT_EQ(cardano_script_any_from_cbor(reader, &script), CARDANO_SUCCESS);
...
/**
 * Releases a script_any and a hex string returned by script_any_to_cbor_hex.
 *
 * @param script The script_any to release.
 * @param hex The hex string to release.
 */
static void
script_any_unref_and_free(cardano_script_any_t** script, char* hex)
{
  cardano_script_any_unref(script);
  free(hex);
}
...
TEST(cardano_script_any_to_cip116_json, canSerializeAny)
{
  // Arrange
  cardano_script_any_t*  any    = NULL;
  ...
  // Act
  error = cardano_script_any_to_cip116_json(any, writer);
  ASSERT_EQ(error, CARDANO_SUCCESS);
...
TEST(cardano_script_any_to_cip116_json, returnsErrorIfNOfKIsNull)
{
  // Arrange
  cardano_json_writer_t* writer = cardano_json_writer_new(CARDANO_JSON_FORMAT_PRETTY);
  ASSERT_THAT(writer, testing::Not((cardano_json_writer_t*)nullptr));
  ...
  // Act
  cardano_error_t error = cardano_script_any_to_cip116_json(nullptr, writer);
...
TEST(cardano_script_any_from_cbor, preservesTheOriginalCborEncoding)
{
  // Arrange
  cardano_script_any_t* script = script_any_from_cbor_hex(NON_MINIMAL_ANY_CBOR);

  // Act
  char* hex = script_any_to_cbor_hex(script);

  // Assert
  EXPECT_STREQ(hex, NON_MINIMAL_ANY_CBOR);

  // Cleanup
  script_any_unref_and_free(&script, hex);
}

TEST(cardano_script_any_set_scripts, discardsTheCachedCbor)
{
  // Arrange
  cardano_script_any_t* script = script_any_from_cbor_hex(NON_MINIMAL_ANY_CBOR);
  ...
  EXPECT_EQ(cardano_script_any_get_scripts(script, &scripts), CARDANO_SUCCESS);
  ...
  EXPECT_EQ(cardano_script_any_set_scripts(script, new_scripts), CARDANO_SUCCESS);
  ...
  // Assert
  EXPECT_STREQ(hex, "82028182041b0000000000000005");

  // Cleanup
  script_any_unref_and_free(&script, hex);
}

TEST(cardano_script_any_clear_cbor_cache, encodesTheScriptFromItsFields)
{
  // Arrange
  cardano_script_any_t* script = script_any_from_cbor_hex(NON_MINIMAL_ANY_CBOR);

  // Act
  cardano_script_any_clear_cbor_cache(script);
  char* hex = script_any_to_cbor_hex(script);

  cardano_script_any_clear_cbor_cache(script);
  char* hex_after_second_clear = script_any_to_cbor_hex(script);

  // Assert
  EXPECT_STREQ(hex, MINIMAL_ANY_CBOR);
  EXPECT_STREQ(hex_after_second_clear, MINIMAL_ANY_CBOR);
...
TEST(cardano_script_any_clear_cbor_cache, doesntCrashIfGivenANullPtr)
{
  // Act
  cardano_script_any_clear_cbor_cache(nullptr);
}

TEST(cardano_script_any_from_cbor, returnsErrorIfMemoryAllocationFailsWhileCachingTheCbor)
{
  // Arrange
  cardano_cbor_reader_t* reader    = cardano_cbor_reader_from_hex(NON_MINIMAL_ANY_CBOR, strlen(NON_MINIMAL_ANY_CBOR));
  ...
  for (int i = 0; (i < 500) && !succeeded; ++i)
  {
    ...
    cardano_error_t result = cardano_script_any_from_cbor(reader_copy, &script);
...
TEST(cardano_script_any_from_cbor, keepsTheOriginalCborOfEachSubScript)
{
  // Arrange
  cardano_script_any_t*         script  = script_any_from_cbor_hex(NON_MINIMAL_ANY_CBOR);
  cardano_native_script_list_t* scripts = NULL;
...
TEST(cardano_script_any_get_scripts, changingASubScriptNeedsAClearOfTheCache)
{
  // Arrange
  cardano_script_any_t*            script         = script_any_from_cbor_hex(NON_MINIMAL_ANY_CBOR);
  cardano_native_script_list_t*    scripts        = NULL;
  ...
  char* hex_before_clear = script_any_to_cbor_hex(script);

  cardano_script_any_clear_cbor_cache(script);
  char* hex = script_any_to_cbor_hex(script);

  // Assert
  EXPECT_STREQ(hex_before_clear, NON_MINIMAL_ANY_CBOR);
  EXPECT_STREQ(hex, "820282820409820507");
...
TEST(cardano_script_any_from_cbor, preservesANonMinimalListHeader)
{
  // Arrange
  cardano_script_any_t* script = script_any_from_cbor_hex("82029802820405820507");

  // Act
  char* hex = script_any_to_cbor_hex(script);

  // Assert
  EXPECT_STREQ(hex, "82029802820405820507");
