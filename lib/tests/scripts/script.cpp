...
TEST(cardano_script_new_plutus_v4, canCreatePlutusV4Script)
{
  // Arrange
  cardano_plutus_v4_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;

  // Act
  cardano_error_t result = cardano_plutus_v4_script_new_bytes_from_hex(PLUTUS_V4_SCRIPT, strlen(PLUTUS_V4_SCRIPT), &plutus_script);

  ASSERT_EQ(result, CARDANO_SUCCESS);

  result = cardano_script_new_plutus_v4(plutus_script, &script);
  ...
  // Cleanup
  cardano_script_unref(&script);
  cardano_plutus_v4_script_unref(&plutus_script);
}

TEST(cardano_script_new_plutus_v4, returnsErrorIfPlutusV4ScriptIsNull)
{
  // Arrange
  cardano_script_t* script = NULL;

  // Act
  cardano_error_t result = cardano_script_new_plutus_v4(NULL, &script);
...
TEST(cardano_script_new_plutus_v4, returnsErrorIfScriptIsNull)
{
  // Arrange
  cardano_plutus_v4_script_t* plutus_script = NULL;
  ...
  result = cardano_script_new_plutus_v4(plutus_script, NULL);
...
TEST(cardano_script_new_plutus_v4, returnsErrorIfMemoryAllocationFails)
{
  // Arrange
  cardano_plutus_v4_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;
...
TEST(cardano_script_from_cbor, canCreatePlutusV4ScriptFromCBOR)
{
  // Arrange
  cardano_script_t*      script = NULL;
  ...
  ASSERT_EQ(cardano_script_get_language(script, &language), CARDANO_SUCCESS);
  EXPECT_EQ(language, CARDANO_SCRIPT_LANGUAGE_PLUTUS_V4);
  ...
  EXPECT_EQ(cardano_blake2b_hash_to_hex(hash, hex, hex_size), CARDANO_SUCCESS);
  EXPECT_STREQ(hex, PLUTUS_V4_HASH);
...
TEST(cardano_script_to_cbor, canConvertPlutusV4ScriptToCBOR)
{
  // Arrange
  cardano_script_t*      script = NULL;
  cardano_cbor_reader_t* reader = cardano_cbor_reader_from_hex(PLUTUS_V4_CBOR, strlen(PLUTUS_V4_CBOR));
  ...
  // Assert
  EXPECT_STREQ(hex, PLUTUS_V4_CBOR);
...
TEST(cardano_script_to_plutus_v4, canConvertScriptToPlutusV4Script)
{
  // Arrange
  cardano_plutus_v4_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;
  ...
  cardano_plutus_v4_script_t* plutus_script2 = NULL;

  result = cardano_script_to_plutus_v4(script, &plutus_script2);
  ...
  // Cleanup
  cardano_plutus_v4_script_unref(&plutus_script2);
  cardano_script_unref(&script);
  cardano_plutus_v4_script_unref(&plutus_script);
}

TEST(cardano_script_to_plutus_v4, returnsErrorIfScriptIsNull)
{
  // Arrange
  cardano_plutus_v4_script_t* plutus_script = NULL;

  // Act
  cardano_error_t result = cardano_script_to_plutus_v4(NULL, &plutus_script);
...
TEST(cardano_script_to_plutus_v4, returnsErrorIfPlutusV4ScriptIsNull)
{
  // Arrange
  cardano_plutus_v4_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;
  ...
  result = cardano_script_to_plutus_v4(script, NULL);
...
TEST(cardano_script_to_plutus_v4, returnsErrorIfNotPlutusV4Script)
{
  // Arrange
  cardano_script_t*           script        = NULL;
  cardano_plutus_v1_script_t* plutus_script = NULL;
...
TEST(cardano_script_equals, returnsTrueIfSamePlutusV4Script)
{
  // Arrange
  cardano_plutus_v4_script_t* plutus_script = NULL;
  cardano_script_t*           script1       = NULL;
  ...
  result = cardano_script_new_plutus_v4(plutus_script, &script1);

  ASSERT_EQ(result, CARDANO_SUCCESS);

  result = cardano_script_new_plutus_v4(plutus_script, &script2);
...
TEST(cardano_script_to_cip116_json, canConvertToNativeScriptToJson)
{
  // Arrange
  cardano_native_script_t* native_script = NULL;
  cardano_script_t*        script        = NULL;
  ...
  result = cardano_script_to_native(script, &native_script1);

  // Assert
  ASSERT_EQ(result, CARDANO_SUCCESS);
  ASSERT_NE(native_script1, nullptr);

  result = cardano_script_to_cip116_json(script, writer);

  ASSERT_EQ(result, CARDANO_SUCCESS);

  char*  buffer = NULL;
  size_t size   = cardano_json_writer_get_encoded_size(writer);

  buffer = (char*)malloc(size);

  result = cardano_json_writer_encode(writer, buffer, size);
  ASSERT_EQ(result, CARDANO_SUCCESS);
...
TEST(cardano_script_to_cip116_json, canConvertToPlutusV1ScriptToJson)
{
  // Arrange
  cardano_plutus_v1_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;
  ...
  cardano_plutus_v1_script_t* plutus_script1 = NULL;

  result = cardano_script_to_plutus_v1(script, &plutus_script1);

  // Assert
  ASSERT_EQ(result, CARDANO_SUCCESS);
  ASSERT_NE(plutus_script1, nullptr);
  ...
  // Cleanup
  cardano_plutus_v1_script_unref(&plutus_script1);
  cardano_script_unref(&script);
  cardano_plutus_v1_script_unref(&plutus_script);
  cardano_json_writer_unref(&writer);
  free(buffer);
...
TEST(cardano_script_to_cip116_json, canConvertToPlutusV2ScriptToJson)
{
  // Arrange
  cardano_plutus_v2_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;
  ...
  const char* expected_json =
    ...
    "  \"value\": {\n"
    "    \"language\": \"plutus_v2\",\n"
    "    \"bytes\": \"\"\n"
    "  }"
    "\n}";
...
TEST(cardano_script_to_cip116_json, canConvertToPlutusV3ScriptToJson)
{
  // Arrange
  cardano_plutus_v3_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;
...
TEST(cardano_script_to_cip116_json, returnsErrorIfScriptIsNull)
{
  // Arrange
  cardano_json_writer_t* writer = cardano_json_writer_new(CARDANO_JSON_FORMAT_PRETTY);

  // Act
  cardano_error_t result = cardano_script_to_cip116_json(NULL, writer);
...
TEST(cardano_script_to_cip116_json, returnsErrorIfWriterIsNull)
{
  // Arrange
  cardano_plutus_v3_script_t* plutus_script = NULL;
  cardano_script_t*           script        = NULL;
  ...
  result = cardano_script_to_cip116_json(script, NULL);
