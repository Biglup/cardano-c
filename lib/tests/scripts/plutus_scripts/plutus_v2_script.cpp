...
TEST(cardano_plutus_v2_script_to_cip116_json, canSerializePlutusV1Script)
{
  // Arrange
  ...
  cardano_error_t error = cardano_plutus_v2_script_new_bytes(SCRIPT_BYTES, sizeof(SCRIPT_BYTES), &script);
  ASSERT_EQ(error, CARDANO_SUCCESS);

  // Act
  error = cardano_plutus_v2_script_to_cip116_json(script, json);
  ...
  EXPECT_STREQ(json_str, "{\"language\":\"plutus_v2\",\"bytes\":\"01020304\"}");
...
TEST(cardano_plutus_v2_script_to_cip116_json, returnsErrorIfScriptIsNull)
{
  // Arrange
  cardano_json_writer_t* json = cardano_json_writer_new(CARDANO_JSON_FORMAT_COMPACT);

  // Act
  cardano_error_t error = cardano_plutus_v2_script_to_cip116_json(nullptr, json);
...
TEST(cardano_plutus_v2_script_to_cip116_json, returnsErrorIfWriterIsNull)
{
  // Arrange
  cardano_plutus_v2_script_t* script         = nullptr;
  const byte_t                SCRIPT_BYTES[] = { 0x01 };

  ASSERT_EQ(cardano_plutus_v2_script_new_bytes(SCRIPT_BYTES, sizeof(SCRIPT_BYTES), &script), CARDANO_SUCCESS);

  // Act
  cardano_error_t error = cardano_plutus_v2_script_to_cip116_json(script, nullptr);
