...
TEST(cardano_plutus_v3_script_list_to_cip116_json, returnsErrorIfGivenNullList)
{
  // Arrange
  cardano_json_writer_t* json = cardano_json_writer_new(CARDANO_JSON_FORMAT_COMPACT);

  // Act
  const cardano_error_t err = cardano_plutus_v3_script_list_to_cip116_json(nullptr, json);
...
TEST(cardano_plutus_v3_script_list_to_cip116_json, returnsErrorIfGivenNullWriter)
{
  // Arrange
  cardano_plutus_v3_script_list_t* list = nullptr;
  ASSERT_EQ(cardano_plutus_v3_script_list_new(&list), CARDANO_SUCCESS);

  // Act
  const cardano_error_t err = cardano_plutus_v3_script_list_to_cip116_json(list, nullptr);
...
TEST(cardano_plutus_v3_script_list_to_cip116_json, canSerializeEmptyList)
{
  // Arrange
  cardano_json_writer_t*           json = cardano_json_writer_new(CARDANO_JSON_FORMAT_COMPACT);
  cardano_plutus_v3_script_list_t* list = nullptr;
  ...
  // Act
  const cardano_error_t err = cardano_plutus_v3_script_list_to_cip116_json(list, json);
...
TEST(cardano_plutus_v3_script_list_to_cip116_json, canSerializeSingleScript)
{
  // Arrange
  cardano_json_writer_t*           json = cardano_json_writer_new(CARDANO_JSON_FORMAT_COMPACT);
  cardano_plutus_v3_script_list_t* list = nullptr;
  ...
  cardano_plutus_v3_script_t* s1       = nullptr;
  ASSERT_EQ(cardano_plutus_v3_script_new_bytes(BYTES1, sizeof(BYTES1), &s1), CARDANO_SUCCESS);
  ASSERT_EQ(cardano_plutus_v3_script_list_add(list, s1), CARDANO_SUCCESS);
  cardano_plutus_v3_script_unref(&s1); // list holds its own ref
  ...
  char* json_str = encode_json(json);
  EXPECT_STREQ(json_str, R"([{"language":"plutus_v3","bytes":"deadbeef"}])");
...
TEST(cardano_plutus_v3_script_list_to_cip116_json, canSerializeMultipleScripts)
{
  // Arrange
  cardano_json_writer_t*           json = cardano_json_writer_new(CARDANO_JSON_FORMAT_COMPACT);
  cardano_plutus_v3_script_list_t* list = nullptr;
  ...
  cardano_plutus_v3_script_t* s1 = nullptr;
  cardano_plutus_v3_script_t* s2 = nullptr;

  ASSERT_EQ(cardano_plutus_v3_script_new_bytes(BYTES1, sizeof(BYTES1), &s1), CARDANO_SUCCESS);
  ASSERT_EQ(cardano_plutus_v3_script_new_bytes(BYTES2, sizeof(BYTES2), &s2), CARDANO_SUCCESS);

  ASSERT_EQ(cardano_plutus_v3_script_list_add(list, s1), CARDANO_SUCCESS);
  ASSERT_EQ(cardano_plutus_v3_script_list_add(list, s2), CARDANO_SUCCESS);

  // Release local refs; the list retains them
  cardano_plutus_v3_script_unref(&s1);
  cardano_plutus_v3_script_unref(&s2);
  ...
  char* json_str = encode_json(json);
  EXPECT_STREQ(json_str, R"([{"language":"plutus_v3","bytes":"0001"},{"language":"plutus_v3","bytes":"aabbcc"}])");
...
TEST(cardano_plutus_v3_script_list_to_cip116_json, returnsErrorIfEventualMemoryAllocationFails)
{
  // Arrange
  cardano_json_writer_t*           json = cardano_json_writer_new(CARDANO_JSON_FORMAT_COMPACT);
  cardano_plutus_v3_script_list_t* list = nullptr;
...
TEST(cardano_plutus_v3_script_list_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_plutus_v3_script_list_t* list    = nullptr;
  cardano_plutus_v3_script_t*      element = new_default_plutus_v3_script(PLUTUS_V3_SCRIPT1_CBOR);

  EXPECT_EQ(cardano_plutus_v3_script_list_new(&list), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_plutus_v3_script_list_add(list, element), CARDANO_SUCCESS);
  }

  const size_t ref_count = cardano_plutus_v3_script_refcount(element);
  ...
  // Act
  cardano_error_t result = cardano_plutus_v3_script_list_add(list, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_plutus_v3_script_list_get_length(list), 127U);
  EXPECT_EQ(cardano_plutus_v3_script_refcount(element), ref_count);
...
TEST(cardano_plutus_v3_script_list_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_plutus_v3_script_list_t* list    = nullptr;
  cardano_plutus_v3_script_t*      element = new_default_plutus_v3_script(PLUTUS_V3_SCRIPT1_CBOR);
  ...
  EXPECT_EQ(cardano_plutus_v3_script_list_to_cbor(list, writer), CARDANO_SUCCESS);
  ...
  cardano_plutus_v3_script_list_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_plutus_v3_script_list_from_cbor(reader, &decoded);
