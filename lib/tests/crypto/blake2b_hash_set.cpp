...
TEST(cardano_blake2b_hash_set_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_blake2b_hash_set_t* set     = nullptr;
  cardano_blake2b_hash_t*     element = new_default_blake2b_hash(BLAKE2B_HASH1_CBOR);

  EXPECT_EQ(cardano_blake2b_hash_set_new(&set), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_blake2b_hash_set_add(set, element), CARDANO_SUCCESS);
  }
  ...
  // Act
  cardano_error_t result = cardano_blake2b_hash_set_add(set, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_blake2b_hash_set_get_length(set), 127U);
  EXPECT_EQ(cardano_blake2b_hash_refcount(element), ref_count);

  // Cleanup
  cardano_set_allocators(malloc, realloc, free);
  reset_limited_realloc();
  cardano_blake2b_hash_set_unref(&set);
  cardano_blake2b_hash_unref(&element);
}

TEST(cardano_blake2b_hash_set_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_blake2b_hash_set_t* set     = nullptr;
  cardano_blake2b_hash_t*     element = new_default_blake2b_hash(BLAKE2B_HASH1_CBOR);
  ...
  EXPECT_EQ(cardano_blake2b_hash_set_to_cbor(set, writer), CARDANO_SUCCESS);
  ...
  cardano_blake2b_hash_set_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_blake2b_hash_set_from_cbor(reader, &decoded);
