...
TEST(cardano_committee_members_map_insert, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_committee_members_map_t* map = nullptr;
  ...
  for (size_t i = 0U; i < 31U; ++i)
  {
    ...
    EXPECT_EQ(cardano_committee_members_map_insert(map, credential, 1), CARDANO_SUCCESS);
  ...
  // Act
  cardano_error_t result = cardano_committee_members_map_insert(map, credential, 1);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_committee_members_map_get_length(map), 31U);
...
TEST(cardano_committee_members_map_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_committee_members_map_t* map = nullptr;
  ...
  EXPECT_EQ(cardano_committee_members_map_to_cbor(map, writer), CARDANO_SUCCESS);
  ...
  cardano_committee_members_map_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_committee_members_map_from_cbor(reader, &decoded);
