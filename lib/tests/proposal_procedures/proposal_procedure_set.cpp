...
TEST(cardano_proposal_procedure_set_add, returnsErrorIfGrowingTheArrayFails)
{
  // Arrange
  cardano_proposal_procedure_set_t* set     = nullptr;
  cardano_proposal_procedure_t*     element = new_default_proposal_procedure(PROPOSAL_PROCEDURE1_CBOR);

  EXPECT_EQ(cardano_proposal_procedure_set_new(&set), CARDANO_SUCCESS);

  for (size_t i = 0U; i < 127U; ++i)
  {
    EXPECT_EQ(cardano_proposal_procedure_set_add(set, element), CARDANO_SUCCESS);
  }

  const size_t ref_count = cardano_proposal_procedure_refcount(element);
  ...
  // Act
  cardano_error_t result = cardano_proposal_procedure_set_add(set, element);

  // Assert
  EXPECT_EQ(result, CARDANO_ERROR_MEMORY_ALLOCATION_FAILED);
  EXPECT_EQ(cardano_proposal_procedure_set_get_length(set), 127U);
  EXPECT_EQ(cardano_proposal_procedure_refcount(element), ref_count);
...
TEST(cardano_proposal_procedure_set_from_cbor, returnsErrorIfGrowingTheArrayFailsWhileDecoding)
{
  // Arrange
  cardano_proposal_procedure_set_t* set     = nullptr;
  cardano_proposal_procedure_t*     element = new_default_proposal_procedure(PROPOSAL_PROCEDURE1_CBOR);
  ...
  EXPECT_EQ(cardano_proposal_procedure_set_to_cbor(set, writer), CARDANO_SUCCESS);
  ...
  cardano_proposal_procedure_set_t* decoded = nullptr;
  ...
  // Act
  cardano_error_t result = cardano_proposal_procedure_set_from_cbor(reader, &decoded);
