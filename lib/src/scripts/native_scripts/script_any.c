...
cardano_error_t
cardano_script_any_to_cip116_json(
  const cardano_script_any_t* script_any,
  cardano_json_writer_t*      writer)
{
  if ((script_any == NULL) || (writer == NULL))
  {
    return CARDANO_ERROR_POINTER_IS_NULL;
  }
  ...
  cardano_json_writer_write_string(writer, "any", 3);
  cardano_json_writer_write_property_name(writer, "scripts", 7);

  cardano_error_t error = cardano_native_script_list_to_cip116_json(script_any->scripts, writer);
