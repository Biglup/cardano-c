...
/**
 * \brief Decodes the fields of a script_pubkey from a CBOR reader.
 *
 * This function reads the CBOR encoded script_pubkey at the current position of the reader and creates a new
 * \ref cardano_script_pubkey_t object from its fields. It does not cache the original CBOR representation, that is done
 * by \ref cardano_script_pubkey_from_cbor.
 *
 * \param[in] reader A pointer to an initialized \ref cardano_cbor_reader_t positioned at the script_pubkey.
 * \param[out] script_pubkey On success, set to the newly created \ref cardano_script_pubkey_t object.
 *
 * \return \ref CARDANO_SUCCESS if the script_pubkey was decoded, or an appropriate error code otherwise.
 */
static cardano_error_t
cardano_script_pubkey_decode(cardano_cbor_reader_t* reader, cardano_script_pubkey_t** script_pubkey)
{
...
cardano_script_pubkey_from_cbor(cardano_cbor_reader_t* reader, cardano_script_pubkey_t** script_pubkey)
{
  ...
  result = cardano_script_pubkey_decode(reader, script_pubkey);
  ...
  (*script_pubkey)->cbor_cache = cbor_cache;
  ...
  cardano_cbor_writer_t*         writer)
{
  ...
  if (script_pubkey->cbor_cache != NULL)
  {
    return cardano_cbor_writer_write_encoded(writer, cardano_buffer_get_data(script_pubkey->cbor_cache), cardano_buffer_get_size(script_pubkey->cbor_cache));
  }
...
cardano_script_pubkey_set_key_hash(cardano_script_pubkey_t* script_pubkey, cardano_blake2b_hash_t* key_hash)
{
  ...
  cardano_buffer_unref(&script_pubkey->cbor_cache);
  script_pubkey->cbor_cache = NULL;
...
void
cardano_script_pubkey_clear_cbor_cache(cardano_script_pubkey_t* script_pubkey)
{
  if (script_pubkey == NULL)
  {
    return;
