...
cardano_error_t
cardano_plutus_v1_script_to_cip116_json(
  const cardano_plutus_v1_script_t* plutus_v1_script,
  cardano_json_writer_t*            writer)
{
  if ((plutus_v1_script == NULL) || (writer == NULL))
  {
    return CARDANO_ERROR_POINTER_IS_NULL;
  }
  ...
  cardano_json_writer_write_string(writer, "plutus_v1", 9);

  cardano_json_writer_write_property_name(writer, "bytes", 5);

  const size_t hex_size = cardano_buffer_get_hex_size(plutus_v1_script->compiled_code);
  ...
  const cardano_error_t to_hex_result =
    cardano_buffer_to_hex(plutus_v1_script->compiled_code, hex, hex_size);
