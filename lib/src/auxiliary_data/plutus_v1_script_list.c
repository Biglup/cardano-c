  ...
  cardano_json_writer_t*                 writer)
{
  ...
  const size_t count = cardano_plutus_v1_script_list_get_length(plutus_v1_scripts);

  for (size_t i = 0; i < count; ++i)
  {
    cardano_plutus_v1_script_t* script = NULL;

    cardano_error_t error = cardano_plutus_v1_script_list_get(plutus_v1_scripts, i, &script);
