...
#include "./internals/enterprise_addr_pack.h"
...
cardano_error_t
cardano_enterprise_address_from_credentials(
  ...
  cardano_enterprise_address_t** enterprise_address)
{
  ...
  address->type =
    ...
    ? CARDANO_ADDRESS_TYPE_ENTERPRISE_KEY
    : CARDANO_ADDRESS_TYPE_ENTERPRISE_SCRIPT;
  ...
  *address->network_id        = network_id;
  ...
  _cardano_pack_enterprise_address(address, address->address_data, sizeof(address->address_data));
  ...
  *enterprise_address = _cardano_from_address_to_enterprise(address);

  return CARDANO_SUCCESS;
}

cardano_error_t
cardano_enterprise_address_from_address(
  ...
  cardano_enterprise_address_t** enterprise_address)
{
  ...
  *enterprise_address = _cardano_from_address_to_enterprise(address_copy);

  return CARDANO_SUCCESS;
}

cardano_address_t*
cardano_enterprise_address_to_address(const cardano_enterprise_address_t* enterprise_address)
{
  if (enterprise_address == NULL)
  {
    return NULL;
  ...
  const cardano_address_t* address = _cardano_from_enterprise_to_address_const(enterprise_address);
...
cardano_enterprise_address_get_payment_credential(
  cardano_enterprise_address_t* enterprise_address)
{
  if (enterprise_address == NULL)
  {
  ...
  cardano_address_t* address = _cardano_from_enterprise_to_address(enterprise_address);
...
cardano_error_t
cardano_enterprise_address_from_bytes(
  const byte_t*                  data,
  const size_t                   size,
...
size_t
cardano_enterprise_address_get_bytes_size(const cardano_enterprise_address_t* address)
{
  return cardano_address_get_bytes_size(_cardano_from_enterprise_to_address_const(address));
}

const byte_t*
cardano_enterprise_address_get_bytes(const cardano_enterprise_address_t* address)
{
  return cardano_address_get_bytes(_cardano_from_enterprise_to_address_const(address));
}

cardano_error_t
cardano_enterprise_address_to_bytes(
  const cardano_enterprise_address_t* address,
  byte_t*                             data,
  const size_t                        size)
{
  return cardano_address_to_bytes(_cardano_from_enterprise_to_address_const(address), data, size);
}
...
size_t
cardano_enterprise_address_get_bech32_size(const cardano_enterprise_address_t* address)
{
  return cardano_address_get_string_size(_cardano_from_enterprise_to_address_const(address));
}

cardano_error_t
cardano_enterprise_address_to_bech32(
  const cardano_enterprise_address_t* address,
  char*                               data,
  const size_t                        size)
{
  return cardano_address_to_string(_cardano_from_enterprise_to_address_const(address), data, size);
}

const char*
cardano_enterprise_address_get_string(const cardano_enterprise_address_t* address)
{
...
{
  cardano_address_unref(_cardano_from_enterprise_pointer_to_address_pointer(address));
}

void
cardano_enterprise_address_ref(cardano_enterprise_address_t* address)
{
  cardano_address_ref(_cardano_from_enterprise_to_address(address));
}

size_t
cardano_enterprise_address_refcount(const cardano_enterprise_address_t* address)
{
...
{
  return cardano_address_set_last_error(_cardano_from_enterprise_to_address(address), message);
}

const char*
cardano_enterprise_address_get_last_error(const cardano_enterprise_address_t* address)
{
  return cardano_address_get_last_error(_cardano_from_enterprise_to_address_const(address));
}