...
_cardano_unpack_enterprise_address(const byte_t* data, size_t size, cardano_enterprise_address_t** address)
{
  ...
  cardano_error_t enterprise_address_result = cardano_enterprise_address_from_credentials(
    network_id,
    payment_credential,
    address);
  ...
  return enterprise_address_result;
}

void
_cardano_pack_enterprise_address(const cardano_address_t* address, byte_t* data, const size_t size)
{
  assert(address != NULL);
  assert(data != NULL);
  assert(size >= (size_t)((size_t)ADDRESS_HEADER_SIZE + (size_t)CARDANO_BLAKE2B_HASH_SIZE_224));
