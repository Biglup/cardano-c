 ...
 */
typedef struct cardano_vote_delegation_cert_t
{
    cardano_object_t      base;
    cardano_credential_t* credential;
    cardano_drep_t*       drep;
} cardano_vote_delegation_cert_t;

/* STATIC FUNCTIONS **********************************************************/

/**
 * \brief Deallocates a vote_delegation_cert object.
 *
 * This function is responsible for properly deallocating a vote_delegation_cert object (`cardano_vote_delegation_cert_t`)
 * and its associated resources.
 *
 * \param object A void pointer to the vote_delegation_cert object to be deallocated. The function casts this
 *               pointer to the appropriate type (`cardano_vote_delegation_cert_t*`).
 *
 * \note It is assumed that this function is called only when the reference count of the vote_delegation_cert
 *       object reaches zero, as part of the reference counting mechanism implemented for managing the
 *       lifecycle of these objects.
 */
static void
cardano_vote_delegation_cert_deallocate(void* object)
{
  assert(object != NULL);

  cardano_vote_delegation_cert_t* data = (cardano_vote_delegation_cert_t*)object;
  ...
  cardano_vote_delegation_cert_t** vote_delegation)
{
  ...
  cardano_vote_delegation_cert_t* data = _cardano_malloc(sizeof(cardano_vote_delegation_cert_t));
  ...
  data->base.deallocator   = cardano_vote_delegation_cert_deallocate;
...
cardano_vote_delegation_cert_from_cbor(cardano_cbor_reader_t* reader, cardano_vote_delegation_cert_t** vote_delegation_cert)
{
  ...
  if (vote_delegation_cert == NULL)
  {
    return CARDANO_ERROR_POINTER_IS_NULL;
  }

  static const char* validator_name = "vote_delegation_cert";
  ...
  const cardano_error_t read_uint_result = cardano_cbor_validate_enum_value(
    ...
    "type",
    reader,
    CARDANO_CERT_TYPE_VOTE_DELEGATION,
  ...
  cardano_error_t new_result = cardano_vote_delegation_cert_new(credential, drep, vote_delegation_cert);
...
cardano_error_t
cardano_vote_delegation_cert_to_cbor(
  ...
  cardano_cbor_writer_t*                writer)
{
  ...
  cardano_error_t write_type_result = cardano_cbor_writer_write_uint(writer, CARDANO_CERT_TYPE_VOTE_DELEGATION);
  ...
  cardano_error_t write_credential_result = cardano_credential_to_cbor(vote_delegation_cert->credential, writer);
  ...
  cardano_error_t write_drep_result = cardano_drep_to_cbor(vote_delegation_cert->drep, writer);
...
cardano_error_t
cardano_vote_delegation_cert_to_cip116_json(
  ...
  cardano_json_writer_t*                writer)
{
  ...
  cardano_json_writer_write_string(writer, "vote_delegation", 15);
...
cardano_vote_delegation_cert_get_credential(cardano_vote_delegation_cert_t* certificate)
{
  if (certificate == NULL)
  {
    return NULL;
...
cardano_drep_t*
cardano_vote_delegation_cert_get_drep(cardano_vote_delegation_cert_t* certificate)
{
  if (certificate == NULL)
  {
    return NULL;
...
void
cardano_vote_delegation_cert_unref(cardano_vote_delegation_cert_t** vote_delegation_cert)
{
  if ((vote_delegation_cert == NULL) || (*vote_delegation_cert == NULL))
  {
    return;
  }

  cardano_object_t* object = &(*vote_delegation_cert)->base;
  cardano_object_unref(&object);

  if (object == NULL)
  {
    *vote_delegation_cert = NULL;
    return;
  }
}

void
cardano_vote_delegation_cert_ref(cardano_vote_delegation_cert_t* vote_delegation_cert)
{
  if (vote_delegation_cert == NULL)
  {
    return;
  ...
  cardano_object_ref(&vote_delegation_cert->base);
}

size_t
cardano_vote_delegation_cert_refcount(const cardano_vote_delegation_cert_t* vote_delegation_cert)
{
  if (vote_delegation_cert == NULL)
  {
    return 0;
  ...
  return cardano_object_refcount(&vote_delegation_cert->base);
}

void
cardano_vote_delegation_cert_set_last_error(cardano_vote_delegation_cert_t* vote_delegation_cert, const char* message)
{
  cardano_object_set_last_error(&vote_delegation_cert->base, message);
}

const char*
cardano_vote_delegation_cert_get_last_error(const cardano_vote_delegation_cert_t* vote_delegation_cert)
{
  return cardano_object_get_last_error(&vote_delegation_cert->base);
}
