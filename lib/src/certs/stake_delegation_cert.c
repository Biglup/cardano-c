 ...
 */
typedef struct cardano_stake_delegation_cert_t
{
...
/**
 * \brief Deallocates a stake_delegation_cert object.
 *
 * This function is responsible for properly deallocating a stake_delegation_cert object (`cardano_stake_delegation_cert_t`)
 * and its associated resources.
 *
 * \param object A void pointer to the stake_delegation_cert object to be deallocated. The function casts this
 *               pointer to the appropriate type (`cardano_stake_delegation_cert_t*`).
 *
 * \note It is assumed that this function is called only when the reference count of the stake_delegation_cert
 *       object reaches zero, as part of the reference counting mechanism implemented for managing the
 *       lifecycle of these objects.
 */
static void
cardano_stake_delegation_cert_deallocate(void* object)
{
  assert(object != NULL);

  cardano_stake_delegation_cert_t* data = (cardano_stake_delegation_cert_t*)object;
  ...
  cardano_stake_delegation_cert_t** stake_delegation)
{
  ...
  cardano_stake_delegation_cert_t* data = _cardano_malloc(sizeof(cardano_stake_delegation_cert_t));
  ...
  data->base.deallocator   = cardano_stake_delegation_cert_deallocate;
...
cardano_stake_delegation_cert_from_cbor(cardano_cbor_reader_t* reader, cardano_stake_delegation_cert_t** stake_delegation_cert)
{
  ...
  if (stake_delegation_cert == NULL)
  {
    return CARDANO_ERROR_POINTER_IS_NULL;
  }

  static const char* validator_name = "stake_delegation_cert";
  ...
  const cardano_error_t read_uint_result = cardano_cbor_validate_enum_value(
    ...
    "type",
    reader,
    CARDANO_CERT_TYPE_STAKE_DELEGATION,
  ...
  cardano_error_t new_result = cardano_stake_delegation_cert_new(credential, hash, stake_delegation_cert);
  ...
  cardano_cbor_writer_t*                 writer)
{
  ...
  cardano_error_t write_credential_result = cardano_credential_to_cbor(stake_delegation_cert->credential, writer);
  ...
  cardano_error_t write_hash_result = cardano_blake2b_hash_to_cbor(stake_delegation_cert->pool_key_hash, writer);
  ...
  cardano_json_writer_t*                 writer)
{
  ...
  cardano_json_writer_write_string(writer, "stake_delegation", 16);
...
cardano_stake_delegation_cert_get_credential(cardano_stake_delegation_cert_t* certificate)
{
  if (certificate == NULL)
  {
    return NULL;
...
cardano_stake_delegation_cert_get_pool_key_hash(cardano_stake_delegation_cert_t* certificate)
{
  if (certificate == NULL)
  {
    return NULL;
...
void
cardano_stake_delegation_cert_unref(cardano_stake_delegation_cert_t** stake_delegation_cert)
{
  if ((stake_delegation_cert == NULL) || (*stake_delegation_cert == NULL))
  {
    return;
  }

  cardano_object_t* object = &(*stake_delegation_cert)->base;
  cardano_object_unref(&object);

  if (object == NULL)
  {
    *stake_delegation_cert = NULL;
    return;
  }
}

void
cardano_stake_delegation_cert_ref(cardano_stake_delegation_cert_t* stake_delegation_cert)
{
  if (stake_delegation_cert == NULL)
  {
    return;
  ...
  cardano_object_ref(&stake_delegation_cert->base);
}

size_t
cardano_stake_delegation_cert_refcount(const cardano_stake_delegation_cert_t* stake_delegation_cert)
{
  if (stake_delegation_cert == NULL)
  {
    return 0;
  ...
  return cardano_object_refcount(&stake_delegation_cert->base);
}

void
cardano_stake_delegation_cert_set_last_error(cardano_stake_delegation_cert_t* stake_delegation_cert, const char* message)
{
  cardano_object_set_last_error(&stake_delegation_cert->base, message);
}

const char*
cardano_stake_delegation_cert_get_last_error(const cardano_stake_delegation_cert_t* stake_delegation_cert)
{
  return cardano_object_get_last_error(&stake_delegation_cert->base);
}
