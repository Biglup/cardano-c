...
/**
 * \brief Serializes a stake vote registration delegation certificate to CIP-116 JSON.
 *
 * The function writes the full JSON object, including the surrounding braces.
 * Keys are written in the order: "tag", "credential", "pool_keyhash", "drep", "coin".
 *
 * \param[in]  cert    Pointer to a valid \ref cardano_stake_vote_registration_delegation_cert_t.
