...
/**
 * \brief Serializes a vote registration delegation certificate to CIP-116 JSON.
 *
 * The function writes the full JSON object, including the surrounding braces.
 * Keys are written in the order: "tag", "credential", "drep", "coin".
 *
 * \param[in]  cert    Pointer to a valid \ref cardano_vote_registration_delegation_cert_t.
 ...
 */
CARDANO_NODISCARD
CARDANO_EXPORT cardano_error_t cardano_vote_registration_delegation_cert_to_cip116_json(
  const cardano_vote_registration_delegation_cert_t* cert,
