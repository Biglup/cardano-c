...
/**
 * \brief Serializes a stake registration and delegation certificate to CIP-116 JSON.
 *
 * The function writes the full JSON object, including the surrounding braces.
 * Keys are written in the order: "tag", "credential", "pool_keyhash", "coin".
 *
 * \param[in]  cert    Pointer to a valid \ref cardano_stake_registration_delegation_cert_t.
 ...
 * CARDANO_ERROR_ENCODING        If the certificate is malformed or serialization fails.
