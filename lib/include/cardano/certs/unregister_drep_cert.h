...
/**
 * \brief Serializes an unregister DRep certificate to CIP-116 JSON.
 *
 * The function writes the full JSON object, including the surrounding braces.
 * Keys are written in the order: "tag", "drep_credential", "coin".
 *
 * \param[in]  cert    Pointer to a valid \ref cardano_unregister_drep_cert_t.
