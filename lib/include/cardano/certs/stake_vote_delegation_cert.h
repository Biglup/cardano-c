...
/**
 * \brief Serializes a stake vote delegation certificate to CIP-116 JSON.
 *
 * The function writes the full JSON object, including the surrounding braces.
 * Keys are written in the order: "tag", "credential", "pool_keyhash", "drep".
 *
 * \param[in]  cert    Pointer to a valid \ref cardano_stake_vote_delegation_cert_t.
...
CARDANO_NODISCARD
CARDANO_EXPORT cardano_error_t
cardano_stake_vote_delegation_cert_to_cip116_json(
  const cardano_stake_vote_delegation_cert_t* cert,
