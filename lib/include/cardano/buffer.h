 ...
 *
 * This function takes a buffer and converts its contents into a hexadecimal string representation.
 * The resulting string is dynamically allocated and should be freed by the caller to avoid memory leaks.
 *
 * \param[in] buffer The source buffer from which the hex string will be generated.
 *
 * \return On success, a pointer to the null-terminated hexadecimal string representing the buffer's contents.
 *         On failure, due to memory allocation issues, returns NULL. It is the caller's responsibility to
 ...
 *
 * char* hexString = cardano_buffer_to_hex(buffer);
 *
 * if (hexString != NULL)
 * {
