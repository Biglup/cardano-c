 ...
 *         To prevent this, when a script_any object is created using \ref cardano_script_any_from_cbor, it caches the original
 *         CBOR representation internally. When \ref cardano_script_any_to_cbor is called, it will output the cached CBOR.
 *         If the cached CBOR representation is not needed, the client can call \ref cardano_script_any_clear_cbor_cache after the object has been created.
 *
 ...
 *          or to the list returned by \ref cardano_script_any_get_scripts, after this object was decoded with
 *          \ref cardano_script_any_from_cbor cannot discard the CBOR cached by this object, so \ref cardano_script_any_to_cbor would
 *          still output the original bytes. Call \ref cardano_script_any_clear_cbor_cache after such a change so that it is serialized.
 *
 * Usage Example:
 * \code{.c}
...
/**
 * \brief Serializes a native \c any script to CIP-116 JSON.
 *
 * This function writes the JSON representation of a native script of kind
 * "any" into \p writer following the CIP-116.
 *
 ...
 *
 * \param[in]     script_any A valid pointer to an \ref cardano_script_any_t.
 ...
 * \return CARDANO_ERROR_POINTER_IS_NULL \p script_any or \p writer is \c NULL.
 * \return CARDANO_ERROR_ENCODING        A nested script failed to serialize or the writer
 *                                       reported an encoding error.
 */
CARDANO_NODISCARD
CARDANO_EXPORT cardano_error_t cardano_script_any_to_cip116_json(
  const cardano_script_any_t* script_any,
 ...
 *         indicating the failure reason.
 *
 * \note A successful call discards the CBOR that the object cached when it was decoded with \ref cardano_script_any_from_cbor,
 *       so the next call to \ref cardano_script_any_to_cbor encodes the script from its fields.
 *
 * Usage Example:
 * \code{.c}
 * cardano_script_any_t* script_any = cardano_script_any_new(...);
...
/**
 * \brief Clears the cached CBOR representation from a script_any.
 *
 * This function removes the internally cached CBOR data from a \ref cardano_script_any_t object.
 * It is useful when you have modified the script_any after it was created from CBOR using
 * \ref cardano_script_any_from_cbor and you want to ensure that the next serialization reflects
 * the current state of the script_any, rather than using the original cached CBOR.
 * The cached CBOR of every sub-script in the list of the script_any is cleared as well, so the whole script is encoded from
 * its fields.
 *
 * \param[in,out] script_any A pointer to an initialized \ref cardano_script_any_t object
 ...
 * Usage Example:
 * \code{.c}
 * // Assume script_any was created using cardano_script_any_from_cbor
 * cardano_script_any_t* script_any = ...;
 * cardano_native_script_t* new_script = ...;
 * cardano_native_script_list_t* scripts = NULL;
 *
 * cardano_error_t result = cardano_script_any_get_scripts(script_any, &scripts);
 *
 * if (result == CARDANO_SUCCESS)
 * {
 *   // Modify the list of sub-scripts, this does not reach the cache of the script_any
 *   result = cardano_native_script_list_add(scripts, new_script);
 *   cardano_native_script_list_unref(&scripts);
 * }
 *
 * // Clear the CBOR cache to ensure serialization uses the updated script_any
 * cardano_script_any_clear_cbor_cache(script_any);
 *
 * cardano_cbor_writer_t* writer = cardano_cbor_writer_new();
 *
 * result = cardano_script_any_to_cbor(script_any, writer);
 *
 * if (result == CARDANO_SUCCESS)
 * {
 ...
 * \endcode
 */
CARDANO_EXPORT void cardano_script_any_clear_cbor_cache(cardano_script_any_t* script_any);
