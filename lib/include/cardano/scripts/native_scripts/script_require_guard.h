 ...
 *         To prevent this, when a script_require_guard object is created using \ref cardano_script_require_guard_from_cbor, it caches the original
 *         CBOR representation internally. When \ref cardano_script_require_guard_to_cbor is called, it will output the cached CBOR.
 *         If the cached CBOR representation is not needed, the client can call \ref cardano_script_require_guard_clear_cbor_cache after the object has been created.
 *
 ...
 *         indicating the failure reason.
 *
 * \warning A change made to the returned credential after this object was decoded with
 *          \ref cardano_script_require_guard_from_cbor cannot discard the CBOR cached by this object, so
 *          \ref cardano_script_require_guard_to_cbor would still output the original bytes. Call
 *          \ref cardano_script_require_guard_clear_cbor_cache after such a change so that it is serialized,
 *          or set the changed credential with \ref cardano_script_require_guard_set_credential.
 *
 * Usage Example:
 * \code{.c}
 ...
 *         indicating the failure reason.
 *
 * \note A successful call discards the CBOR that the object cached when it was decoded with \ref cardano_script_require_guard_from_cbor,
 *       so the next call to \ref cardano_script_require_guard_to_cbor encodes the script from its fields.
 *
 * Usage Example:
 * \code{.c}
 * cardano_script_require_guard_t* script_require_guard = ...; // Assume script_require_guard is initialized
...
/**
 * \brief Clears the cached CBOR representation from a script_require_guard.
 *
 * This function removes the internally cached CBOR data from a \ref cardano_script_require_guard_t object.
 * It is useful when you have modified the script_require_guard after it was created from CBOR using
 * \ref cardano_script_require_guard_from_cbor and you want to ensure that the next serialization reflects
 * the current state of the script_require_guard, rather than using the original cached CBOR.
 *
 * \param[in,out] script_require_guard A pointer to an initialized \ref cardano_script_require_guard_t object
 ...
 * Usage Example:
 * \code{.c}
 * // Assume script_require_guard was created using cardano_script_require_guard_from_cbor
 * cardano_script_require_guard_t* script_require_guard = ...;
 *
 * // Clear the CBOR cache so that serialization encodes the script from its fields
 * cardano_script_require_guard_clear_cbor_cache(script_require_guard);
 *
 * cardano_cbor_writer_t* writer = cardano_cbor_writer_new();
 *
 * cardano_error_t result = cardano_script_require_guard_to_cbor(script_require_guard, writer);
 ...
 * \endcode
 */
CARDANO_EXPORT void cardano_script_require_guard_clear_cbor_cache(cardano_script_require_guard_t* script_require_guard);
