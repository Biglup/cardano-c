 ...
 *         To prevent this, when a script_n_of_k object is created using \ref cardano_script_n_of_k_from_cbor, it caches the original
 *         CBOR representation internally. When \ref cardano_script_n_of_k_to_cbor is called, it will output the cached CBOR.
 *         If the cached CBOR representation is not needed, the client can call \ref cardano_script_n_of_k_clear_cbor_cache after the object has been created.
 *
 ...
 *          or to the list returned by \ref cardano_script_n_of_k_get_scripts, after this object was decoded with
 *          \ref cardano_script_n_of_k_from_cbor cannot discard the CBOR cached by this object, so \ref cardano_script_n_of_k_to_cbor would
 *          still output the original bytes. Call \ref cardano_script_n_of_k_clear_cbor_cache after such a change so that it is serialized.
 *
 * Usage Example:
 * \code{.c}
 ...
 *
 * \note A successful call discards the CBOR that the object cached when it was decoded with \ref cardano_script_n_of_k_from_cbor,
 *       so the next call to \ref cardano_script_n_of_k_to_cbor encodes the script from its fields.
 *
 * Usage Example:
 * \code{.c}
 * cardano_script_n_of_k_t* script_n_of_k = ...; // Assume this is initialized
...
/**
 * \brief Clears the cached CBOR representation from a script_n_of_k.
 *
 * This function removes the internally cached CBOR data from a \ref cardano_script_n_of_k_t object.
 * It is useful when you have modified the script_n_of_k after it was created from CBOR using
 * \ref cardano_script_n_of_k_from_cbor and you want to ensure that the next serialization reflects
 * the current state of the script_n_of_k, rather than using the original cached CBOR.
 * The cached CBOR of every sub-script in the list of the script_n_of_k is cleared as well, so the whole script is encoded from
 * its fields.
 *
 * \param[in,out] script_n_of_k A pointer to an initialized \ref cardano_script_n_of_k_t object
 ...
 * Usage Example:
 * \code{.c}
 * // Assume script_n_of_k was created using cardano_script_n_of_k_from_cbor
 * cardano_script_n_of_k_t* script_n_of_k = ...;
 * cardano_native_script_t* new_script = ...;
 * cardano_native_script_list_t* scripts = NULL;
 *
 * cardano_error_t result = cardano_script_n_of_k_get_scripts(script_n_of_k, &scripts);
 *
 * if (result == CARDANO_SUCCESS)
 * {
 *   // Modify the list of sub-scripts, this does not reach the cache of the script_n_of_k
 *   result = cardano_native_script_list_add(scripts, new_script);
 *   cardano_native_script_list_unref(&scripts);
 * }
 *
 * // Clear the CBOR cache to ensure serialization uses the updated script_n_of_k
 * cardano_script_n_of_k_clear_cbor_cache(script_n_of_k);
 *
 * cardano_cbor_writer_t* writer = cardano_cbor_writer_new();
 *
 * result = cardano_script_n_of_k_to_cbor(script_n_of_k, writer);
 *
 * if (result == CARDANO_SUCCESS)
 * {
 ...
 * \endcode
 */
CARDANO_EXPORT void cardano_script_n_of_k_clear_cbor_cache(cardano_script_n_of_k_t* script_n_of_k);
