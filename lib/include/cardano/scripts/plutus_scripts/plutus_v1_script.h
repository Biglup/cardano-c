...
/**
 * \brief Serializes a Plutus v1 script into a CIP-116 JSON object.
 *
 ...
 * property name).
 *
 * \param[in]     plutus_v1_script  A valid pointer to the script to serialize.
 * \param[in,out] writer            A valid JSON writer positioned where a value can be written.
 *
 * \return
 * - \c CARDANO_SUCCESS on success.
 * - \c CARDANO_ERROR_POINTER_IS_NULL if \p plutus_v1_script or \p writer is \c NULL.
