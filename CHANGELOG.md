...
- Added `cardano_tx_builder_add_sub_transaction` to the transaction builder, the batcher side of CIP-118: it adds a finished sub transaction to the sub transaction set of the transaction body (key 23) together with the UTXOs that resolve its inputs. The sub transaction is opaque, the builder keeps a reference to it and never modifies it, so its original bytes, its id and its signatures are preserved. A sub transaction whose id is already in the batch, that spends an input already spent by another sub transaction or by an input added with `cardano_tx_builder_add_input`, or whose spend inputs are not all resolved is rejected with a deferred error, and `cardano_tx_builder_add_input` now rejects an input that is already spent by a sub transaction. The resolved UTXOs may also resolve the reference inputs of the sub transaction, whose reference scripts are then paid for by the fee. The batcher must add every top level guard a sub transaction requires with `cardano_tx_builder_add_guard`: the builder never adds them by itself and `cardano_tx_builder_build` fails with `CARDANO_ERROR_ELEMENT_NOT_FOUND` when one is missing.
...
- Fixed `cardano_tx_builder_propose_hardfork_ex` and `cardano_sub_tx_builder_propose_hardfork_ex` building the hard fork initiation action with the major and minor protocol version swapped, so a proposal for version 12.0 was encoded as 0.12. Both functions now produce the same governance action as `cardano_tx_builder_propose_hardfork` and `cardano_sub_tx_builder_propose_hardfork` given the same protocol version; their signatures do not change.
