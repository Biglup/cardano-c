...
- Added the `sub_transaction_batch_example` example and an end to end settlement test suite for CIP-118 batches: two parties express opposite intents as signed sub transactions and a batcher aggregates and balances them. The example runs offline with hardcoded UTXOs and prints the CBOR of the batch. The tests cover a batch that nets to zero, a deficit funded by the batcher and a surplus kept by the batcher as change, check that the embedded sub transactions stay byte identical and their signatures keep verifying, and include a property test over random sets of sub transactions with random ADA and token imbalances.
