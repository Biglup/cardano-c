...
- **BREAKING**: `cardano_coin_selector_select` and the `cardano_coin_select_func_t` callback now also receive an optional `outputs_to_cover` list, used by selectors as a shape and weight hint when generating change outputs.
