...
- Added `cardano_tx_builder_add_sub_transaction` to the transaction builder, the batcher side of CIP-118: it adds a finished sub transaction to the sub transaction set of the transaction body (key 23) together with the UTXOs that resolve its inputs. The sub transaction is opaque, the builder keeps a reference to it and never modifies it, so its original bytes, its id and its signatures are preserved. A sub transaction whose id is already in the batch, that spends an input already spent by another sub transaction or by an input added with `cardano_tx_builder_add_input`, or whose spend inputs are not all resolved is rejected with a deferred error, and `cardano_tx_builder_add_input` now rejects an input that is already spent by a sub transaction. The resolved UTXOs may also resolve the reference inputs of the sub transaction, whose reference scripts are then paid for by the fee.
